...
export async function syncAllToSupabase() {
  ...
  for (const store of STORES) {
    ...
    for (const value of values) {
      await pushRecord(client, store, value);
      pushed += 1;
    }
  }
  const queueResult = await flushSyncQueue();
  const health = await dbHealthCheck();
  return { ok: true, pushed, queueResult, remoteCount: health.remoteCount || 0, pending: health.pending || 0 };
}
